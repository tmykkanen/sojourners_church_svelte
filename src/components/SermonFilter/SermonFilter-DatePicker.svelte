...
<DatePicker.Root weekdayFormat="short" fixedWeeks={true} bind:value>
  <div class="flex w-full">
    <DatePicker.Label
      class="select-none bg-muted rounded-l-md px-4 border border-muted-content/20 h-full flex items-center flex-1/4 sm:flex-1 min-w-24 text-sm"
    ...
    <DatePicker.Content sideOffset={6} class="z-50">
      <DatePicker.Calendar
        class="border-dark-10 bg-muted shadow-popover rounded-md border p-8"
      >
        {#snippet children({ months, weekdays })}
          <DatePicker.Header class="flex items-center justify-between">
            <DatePicker.PrevButton
              class="rounded-9px bg-muted hover:bg-secondary inline-flex size-10 items-center justify-center transition-all active:scale-[0.98]"
            >
                  ...
                  {#each month.weeks as weekDates (weekDates)}
                    <DatePicker.GridRow class="flex w-full">
                      {#each weekDates as date (date)}
                        ...
                        >
                          <DatePicker.Day
                            class="rounded-9px text-foreground hover:border-primary data-selected:bg-foreground data-disabled:text-foreground/30 data-selected:text-background data-unavailable:text-muted-foreground data-disabled:pointer-events-none data-outside-month:pointer-events-none data-selected:font-medium data-unavailable:line-through group relative inline-flex size-10 items-center justify-center whitespace-nowrap border border-transparent bg-transparent p-0 text-sm font-normal transition-all"
                          >
                            <div
